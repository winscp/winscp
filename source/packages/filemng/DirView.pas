...
begin
  ...
  begin
    ...
    if (not FInvalidItem) and (not Terminated) and
       FCurrentItemData.IconEmpty then
    begin
      try
    ...
    if FCurrentIndex = FIndex then Inc(FIndex);
    SetLength(FCurrentFilePath, 0);
  end;
...
begin
  ...
  if not Terminated and
     (Pred(FOwner.Items.Count) >= FCurrentIndex) and
     Assigned(FOwner.Items[FCurrentIndex]) and
     Assigned(FOwner.Items[FCurrentIndex].Data) then
  begin
    FCurrentFilePath := FOwner.ItemFullFileName(FOwner.Items[FCurrentIndex]);
    FCurrentItemData := PFileRec(FOwner.Items[FCurrentIndex].Data)^;
    FInvalidItem := False;
  end
    else FInvalidItem := True;
...
var
  LVI: TLVItem;
begin
  if (FOwner.Items.Count > FCurrentIndex) and
     not FOwner.Loading and not Terminated and
     Assigned(FOwner.Items[FCurrentIndex]) and
     Assigned(FOwner.Items[FCurrentIndex].Data) then
    with FOwner.Items[FCurrentIndex] do
    begin
